...
We have four BAM files associated with this workshop, so that we can 
explore how real RNA-seq fragments look using Bioconductor.
These four files are associated with GEUVADIS RNA-seq samples, 
aligned using the [STAR](https://github.com/alexdobin/STAR) read aligner, 
aligning to hg19. The files contain just a subset of the total reads,
to limit the disk space required for this workshop. The files contain
only the reads that overlap the genes USF2 and CHPF.
